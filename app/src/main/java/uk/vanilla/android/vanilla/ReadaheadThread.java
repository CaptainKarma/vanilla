		...
		switch (message.what) {
			case MSG_SET_SONG: {
				...
				if (mInputStream != null && !mPath.equals(song.path)) {
					...
					try {
						...
						Log.e("VanillaICE", "Failed to close file: "+e);
					}
					mPath = null;
					mReadDelay = 0;
					mInputStream = null;
				...
				if (mInputStream == null) {
					...
					try {
						...
						Log.e("VanillaICE", "Failed to song "+ song +": "+e);
					} catch (IOException e) {
						Log.e("VanillaICE", "IO Exception on "+ song +": "+e);
					}
				}
			...
			case MSG_READ_CHUNK: {
				...
				if (bytesRead >= 0) {
					...
					Log.d("VanillaICE", "Readahead for "+mPath+" finished");
				}
			}
			default: {
				break;
